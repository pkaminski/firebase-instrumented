...
fb.core.WriteTree.prototype.calcCompleteEventCache = function(treePath, completeServerCache, writeIdsToExclude, includeHiddenWrites, opt_log) {
  if (opt_log) {
    fb.core.util.log("WriteTree.calcCompleteEventCache: writes=", goog.array.map(this.allWrites_, function(x) {
      return{"id":x.writeId, "path":x.path.toString()};
    }));
  }
...
fb.core.SyncTree.prototype.calcCompleteEventCache = function(path, writeIdsToExclude, opt_log) {
  ...
  if (opt_log) {
    fb.core.util.log("calcCompleteEventCache_(path=", path.toString(), " writeIdsToExclude=", writeIdsToExclude, ")");
    fb.core.util.log("calcCompleteEventCache_: serverCache=", serverCache && serverCache.val(true));
  }
  return writeTree.calcCompleteEventCache(path, serverCache, writeIdsToExclude, includeHiddenSets, true);
};
...
fb.core.Repo.prototype.getLatestState_ = function(path, excludeSets) {
  return this.serverSyncTree_.calcCompleteEventCache(path, excludeSets, true) || fb.core.snap.EMPTY_NODE;
};
fb.core.Repo.prototype.sendReadyTransactions_ = function(opt_node) {
  var node = (opt_node || this.transactionQueueTree_);
  if (!opt_node) {
    this.pruneCompletedTransactionsBelowNode_(node);
    fb.core.util.log("sendReadyTransactions_: root");
    this.dumpQueues_();
  }
  if (node.getValue() !== null) {
    ...
    if (allRun) {
      fb.core.util.log("sendReadyTransactions_: sending at " + node.path().toString());
...
fb.core.Repo.prototype.dumpQueues_ = function() {
  var queues = {};
  var self = this;
  this.transactionQueueTree_.forEachDescendant(function(tree) {
    var queue = tree.getValue();
    if (queue) {
      queues[tree.path().toString()] = self.prettyQueue_(queue);
    }
  }, true);
  fb.core.util.log("Transaction queues:", queues);
};
fb.core.Repo.prototype.prettyQueue_ = function(queue) {
  var pretty = [];
  for (var i = 0;i < queue.length;i++) {
    var t = queue[i];
    pretty.push({"status":t.status, "applyLocally":t.applyLocally, "writeId":t.currentWriteId, "inputSnapshot":t.currentInputSnapshot && t.currentInputSnapshot.val(true), "outputSnapshotRaw":t.currentOutputSnapshotRaw && t.currentOutputSnapshotRaw.val(true), "outputSnapshotResolved":t.currentOutputSnapshotResolved && t.currentOutputSnapshotResolved.val(true)});
  }
  return pretty;
};
fb.core.Repo.prototype.sendTransactionQueue_ = function(path, queue) {
  fb.core.util.log("sendTransactionQueue_(path=", path.toString(), " queue=", this.prettyQueue_(queue), ")");
  var setsToIgnore = goog.array.map(queue, function(txn) {
    return txn.currentWriteId;
  });
  var latestState = this.getLatestState_(path, setsToIgnore);
  fb.core.util.log("sendTransactionQueue_: setsToIgnore=", setsToIgnore, "latestState=", latestState.val(true));
